...
export function fakePlugin(over: any = {}) {
  ...
  const p: any = {
    ...
    mountStates: () => ({}),
    roMountEditsFor: () => [],                        // issueMountRoLocalEditBehavior: no read-only edits by default
    updateMountDirection: vi.fn(async () => {}),
    keepRoMountEdits: vi.fn(async () => 0),
    reinstateMount: vi.fn(async () => {}),
    pendingBulkDeletions: () => [],                   // D0041: no held incoming deletions by default
