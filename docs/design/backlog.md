...
## B11 — Parallelize chunk transfer (perf, surfaced by the E2E suite 2026-07-04)
The large-E2E suite showed chunk transfer is SEQUENTIAL: `pushFile` awaits `putChunk` per
missing chunk, and `fetchFileBytes` awaits `getChunk` per chunk. An all-unique-content blob is
the worst case (~size/64 KiB round-trips) — a 10 MB unique binary took ~133 s on a Windows host
(FS latency per chunk amplifies it; Linux/docker is far faster). Fix: bound-concurrency the
missing-chunk uploads and the chunk fetches (e.g. Promise pool of ~8). Content-addressing makes
this safe. Not blocking (typical notes are small + dedup-heavy), but it's the clear scaling lever
for large binaries. Test `large binary (~4 MB…)` documents the current cost.
...
- **De-dup the E2E harness** — `client/test/e2e-helpers.ts` is now the shared harness (used by the
  standalone permutation suite); refactor `client/test/e2e.spec.ts` to import it instead of its
  inline copy (left inline for now to avoid destabilizing the passing 21-test suite mid-flight).
