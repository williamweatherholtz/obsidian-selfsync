...
### Standalone / on-demand suites (NOT run on each compile)

Some checks are too heavy or combinatorial for the default gate and are gated behind an env
flag so `npx vitest run` skips them. Run explicitly when relevant:

```
# Exhaustive config-sync dial matrix — all 2^5 permutations of core/hotkeys/appearance/
# snippets/community, verified pure (shouldSync) AND end-to-end (each enabled category's
# file actually tracks to the other vault; off-dials + SelfSync's own folder never do).
cd server && cargo build                 # (or set SYNC_SERVER_URL for a container)
cd ../client && SELFSYNC_PERM=1 npx vitest run test/config-permutations.spec.ts
```

Shared harness lives in `client/test/e2e-helpers.ts` (server spawn + transport + IO +
`connect`), imported by the standalone suites.
