...
| **3.5.3 multifactor authentication** | **DEPRECATED (D0038)** | MFA (TOTP + recovery codes) is **off by default** as of D0038 (2026-08-05): the Obsidian plugin's login can't supply a TOTP code, so enforcing it locked plugin users out — the owner chose to retire MFA rather than build plugin-side TOTP. The implementation (`mfa.rs`/`totp.rs`, RFC 6238, pinned to RFC 4231 vectors) is **retained, dormant, and flag-gated**: `MFA_ENABLED=1` fully restores TOTP + recovery + `REQUIRE_ADMIN_MFA` for a web-admin-only deployment. With MFA off, this control is **not met** for this deployment; the trusted-server + TLS-in-transit posture and all other IA/AC/AU controls are unchanged |
