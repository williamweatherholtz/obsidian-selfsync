...
description: |
  ...
  elements", "rank by criticality", or to work the `keel show arch` frontier.
metadata:
  version: 0.1.0
...
3. **Rank:** `keel show arch criticality` → audit High→Low.
...
8. **Drift:** `keel show arch drift` keeps the registry honest against the in-code hashes.

## keel views (follow-on Rust build in sysmlv2-ai-toolkit; until then compute by hand / audit_hash.py)
`keel show arch elements | criticality | coupling | drift | stpa-inputs | coverage`. `drift` reuses `audit_hash.py`.
